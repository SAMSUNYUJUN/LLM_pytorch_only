...
Compares: GPT-2 tokenizer, GPT-4 (cl100k_base) tokenizer, and ours (saved tokenizer.json).
"""
...
from app.modules.tokenizer.tokenizer import HuggingFaceTokenizer, get_tokenizer
...
train_text = "\n".join(train_docs) if train_docs else ""
val_docs = next(parquets_iter_batched(split="val"))
val_text = "\n".join(val_docs) if val_docs else ""
...
for tokenizer_name in ["gpt2", "gpt4", "ours"]:
    if tokenizer_name == "gpt2":
        ...
        tokenizer = HuggingFaceTokenizer.from_pretrained("cl100k_base")
    else:
    ...
    for name, text in all_text:
        ...
        assert decoded == text, f"[{tokenizer_name}] decode mismatch on {name}"

        encoded_bytes = text.encode("utf-8")
        ratio = len(encoded_bytes) / len(encoded) if len(encoded) > 0 else 0.0
...
def print_comparison(baseline_name, baseline_results, ours_results, all_text):
    ...
    for name, _ in all_text:
        ...
        relative_diff = (
            ((baseline_data["tokens"] - ours_data["tokens"]) / baseline_data["tokens"]) * 100
            if baseline_data["tokens"] > 0
            else 0.0
        )
